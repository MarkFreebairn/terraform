...
func (p *blockBodyDiffPrinter) writeBlockBodyDiff(schema *configschema.Block, old, new cty.Value, indent int) {
...

	{
		...
		for _, name := range blockTypeNames {
			...
			p.writeNestedBlockDiffs(name, blockS, oldVal, newVal, indent)
		}
	}
}
...
func (p *blockBodyDiffPrinter) writeNestedBlockDiffs(name string, blockS *configschema.NestedBlock, old, new cty.Value, indent int) {
...
func (p *blockBodyDiffPrinter) writeNestedBlockDiff(name string, label *string, blockS *configschema.Block, action diffs.Action, old, new cty.Value, indent int) {
	...
	case diffs.Delete:
		p.buf.WriteString(p.color.Color("[red]-[reset] "))
	case diffs.Update, diffs.Replace: // "Replace" is not actually valid for nested blocks
		p.buf.WriteString(p.color.Color("[yellow]~[reset] "))
	default:
		// Assume any other value is NoAction, since the above accounts for
		// all other reasonable values of action.
		p.buf.WriteString(p.color.Color("  "))
	}
	...
	p.writeBlockBodyDiff(blockS, old, new, indent+2)
